...
<body>
    ...
    <div class="container" id="letter">
        ...
        <main>
            ...
            <p>This is to certify that {{ $certificateLetter->user->name ?? '---' }} successfully completed an
                internship at
                {{ env('COMPANY_NAME') }} from
                {{ $certificateLetter->start_date ?? '---' }} to {{ $certificateLetter->end_date ?? '---' }} in the
                {{ $certificateLetter->position ?? '---' }}.</p>


            <p>During the internship period, {{ $certificateLetter->user->name ?? '---' }} actively contributed to
                various
                projects and
            ...
            <p style="margin-top: 10px">{{ $certificateLetter->user->name ?? '---' }} demonstrated excellent
                dedication,
                initiative, and problem-solving
                abilities throughout the internship. The work performed reflected a strong commitment to learning,
                professionalism, and teamwork. {{ $certificateLetter->user->name ?? '---' }} consistently met assigned
                objectives
                and made valuable contributions to the organization.</p>

            <p>Based on performance and conduct during the internship, {{ $certificateLetter->user->name ?? '---' }}
                has shown
            ...
            <P>You are expected to complete e-documentation formalities before you join on
                {{ $certificateLetter->start_date ?? '---' }}. Please
                {{ env('COMPANY_NAME') }}
        ...
        {{-- <button class="btn " style="margin-top: 30px; padding:10px; cursor:pointer" onclick="downloadPDF()">
            Download PDF
        </button> --}}
    </div>
    ...
    {{-- <script>
        async function downloadPDF() {
            ...
            const certificate = document.getElementById("letter");
            ...
            pdf.save("Certificate_Letter.pdf");
        }
    </script> --}}
</body>
