@php
    $customerTypeValue = old('customer_type') ?? ($estimate?->client ? 'client' : 'customer');
    $clientIdValue = old('client_id', $estimate?->client_id ?? '');
    $estimateNumberValue = old('estimate_number', $estimate?->estimate_number ?? ($nextNumber ?? ''));
    $estimateDateValue = old('estimate_date', $estimate?->estimate_date?->toDateString() ?? now()->toDateString());
    $expiryDateValue = old('expiry_date', $estimate?->expiry_date?->toDateString() ?? ($defaultExpiry ?? ''));
    $statusValue = old('status', $estimate?->status ?? array_key_first($statuses));
    $buyerNameValue = old('buyer_name', $estimate?->buyer_name ?? '');
    $buyerMobileValue = old('buyer_mobile', $estimate?->buyer_mobile ?? '');
    $buyerGstinValue = old('buyer_gstin', $estimate?->buyer_gstin ?? '');
...
@endphp
<form action="{{ $formAction }}" method="POST" id="estimate_form">
    @csrf
    @if ($formMethodNormalized !== 'POST')
        @method($formMethodNormalized)
    @endif

    <span class="text-warning fs-4">Estimate Info</span>
    ...
    <div class="row g-3 mb-4">
        <div class="col-12">
            ...
            <div class="d-flex flex-wrap gap-4">
                <div class="form-check">
                    ...
                    <label class="form-check-label" for="customer_type_customer">Customer (enter details
                        manually)</label>
                </div>
                <div class="form-check">
        ...
        <div class="col-md-6 client-select-wrapper {{ $customerTypeValue !== 'client' ? 'd-none' : '' }}">
        ...
        <div class="col-md-6">
            ...
            <input type="text" class="form-control" name="estimate_number"
                value="{{ $estimateNumberValue }}" required>
        </div>
        ...
        <div class="col-md-6  date-field">
            ...
            <input type="date" class="form-control" name="estimate_date"
                value="{{ $estimateDateValue }}" required>
        </div>
        </div>
        <div class="row date-fields-row">
        <div class="col-md-6  date-field">
    ...
    <span class="text-warning fs-4">Estimate Items</span>
    ...
    <span class="text-warning fs-4">Estimate Summary</span>
    ...
    <div class="mt-4 row gx-3">
        ...
        <div class="col-md-4">
            ...
            <div class="fs-5 fw-bold" id="total_tax_display">0.00</div>
            <input type="hidden" name="total_tax" id="total_tax" value="{{ old('total_tax', 0) }}">
        </div>
        <div class="col-md-4">
    ...
    <div class="mt-4 text-end">
        <button type="submit" class="btn btn-primary">{{ $submitLabel ?? 'Save estimate' }}</button>
    </div>
</form>
...
@section('scripts')
    <script>
        (function($) {
            const clientDetailsBase = "{{ url('dashboard/clients') }}";
            ...
            const $itemRowTemplate = $('#item_row_template');
            const itemRowHtml = $itemRowTemplate.length ? $itemRowTemplate.html() : '';
            ...
            function refreshTotals() {
                ...
                $itemsBody.find('tr').each(function() {
                    const $row = $(this);
                    subTotal += $row.data('taxable') || 0;
                    totalTax += $row.data('tax') || 0;
                });
            ...
            function bindRow($row) {
                ...
                $row.find('.product-selector').on('change', function() {
                    applyProductToRow($row, $(this).val());
                });
            ...
            function fetchClientDetails(clientId) {
                ...
                const url = clientDetailsBase + '/' + clientId + '/details';
            ...
            $('#add_item_row').on('click', function() {
                ...
                const rowHtml = itemRowHtml.replace(/__INDEX__/g, index);
                const $row = $(rowHtml);
